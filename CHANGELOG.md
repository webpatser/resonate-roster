...
- Support `webpatser/resonate` v0.7. With scaling enabled, `onSubscribe` and `onClose` on presence channels can land up to ~1s later, because core now queries other nodes first before firing them.
